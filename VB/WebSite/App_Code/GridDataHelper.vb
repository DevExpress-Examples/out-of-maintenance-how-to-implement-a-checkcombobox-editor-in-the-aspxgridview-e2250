...
Public Class GridDataHelper
    Public Shared Function GetData() As List(Of GridDataModel)
        Return Enumerable.Range(0, 100).Select(Function(x) New GridDataModel With { _
            .ID = x, _
            .Browsers = If((x Mod 5) <> 4, GetBrowserNameByID(x) & ";" & GetBrowserNameByID(x + 1), GetBrowserNameByID(x + 1) & ";" & GetBrowserNameByID(x)) _
        }).ToList()
    End Function
    Private Shared Function GetBrowserNameByID(ByVal id As Integer) As String
        Return If((id Mod 5) = 0, "Chrome", If((id Mod 5) = 1, "Firefox", If((id Mod 5) = 2, "IE", If((id Mod 5) = 3, "Opera", "Safari"))))
    End Function
